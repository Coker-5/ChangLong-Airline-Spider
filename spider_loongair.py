...
def do_login(start_data):
    ...
    try:
        ...
        login_paras = {"ActionType": "AccountService_UserLoginHandler", "SessionId": "",
                       "Args": "{\"UserName\":\"%s\",\"Password\":\"%s\",\"Token\":\"\",\"ImgCode\":\"\",\"SaleChannel\":8,\"ConstID\":\"5e4275c4RnTIE51IOyn8PHez13fquwq9VePHCAq1\"}" % (
                       start_data['username'], start_data['password']),
                       "MethodName": "AccountService_UserLoginHandler", "AppID": "ClientService",
                       "Parameters": "{\"UserName\":\"%s\",\"Password\":\"%s\",\"Token\":\"\",\"ImgCode\":\"\",\"SaleChannel\":8,\"ConstID\":\"5e4275c4RnTIE51IOyn8PHez13fquwq9VePHCAq1\"}" % (
                       start_data['username'], start_data['password'])
                       }
...
def get_data(start_data):
    ...
    data_paras = {
        ...
        "Args": "{\"departureAirportCode\":\"%s\",\"arriveAirportCode\":\"%s\",\"departureTime\":\"%s\",\"tripType\":1,\"cross\":\"4565168990772300027072\",\"ifNonCompany\":false,\"SaleChannel\":8,\"ConstID\":\"5e4275c4RnTIE51IOyn8PHez13fquwq9VePHCAq1\"}" % (
        start_data['from'], start_data['to'], start_data['daytime']),
        "MethodName": "searchConnectFlights", "AppID": "ClientService",
        "Parameters": "{\"departureAirportCode\":\"%s\",\"arriveAirportCode\":\"%s\",\"departureTime\":\"%s\",\"tripType\":1,\"cross\":\"4565168990772300027072\",\"ifNonCompany\":false,\"SaleChannel\":8,\"ConstID\":\"5e4275c4RnTIE51IOyn8PHez13fquwq9VePHCAq1\"}" % (
        start_data['from'], start_data['to'], start_data['daytime'])
    }
    ...
    try:
        ...
        if resp.status_code == 200 or resp.status_code == 201:
            ...
            return {"error": f"获取数据出错，状态码{resp.status_code}"}
    except Exception as e:
...
if __name__ == '__main__':
    ...
    do_login(start_data=start_data)

    print(get_data(start_data))
